...
def iMAE_metric(D_est, D_gt, mask):
    iD_est, iD_gt = 1 / (D_est[mask] + 1e-6), 1 / (D_gt[mask])
    iD_est[D_est[mask]==0] = 0
    iD_gt[D_gt[mask]==0] = 0
    E = torch.abs(iD_gt - iD_est)
    return torch.mean(E.float())

@make_nograd_func
@compute_metric_for_each_image
def iRMSE_metric(D_est, D_gt, mask):
    iD_est, iD_gt = 1 /(D_est[mask] + 1e-6), 1 / D_gt[mask]
    iD_est[D_est[mask]==0] = 0
    iD_gt[D_gt[mask]==0] = 0
    E = F.mse_loss(iD_est, iD_gt, reduction="mean")
    return torch.sqrt(E)

@make_nograd_func
@compute_metric_for_each_image
def MAE_metric(D_est, D_gt, mask):
...
@make_nograd_func
@compute_metric_for_each_image
def RMSE_metric(D_est, D_gt, mask):
    D_est, D_gt = D_est[mask], D_gt[mask]
    D_est = D_est.clamp(max=100.0)
    E = F.mse_loss(D_est, D_gt, reduction="mean")
    return torch.sqrt(E)
